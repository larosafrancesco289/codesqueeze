...
describe("shouldIgnoreFile", () => {
  ...
  it("should respect custom ignore patterns for filenames", () => {
    ...
    expect(shouldIgnoreFile("components/Button.test.tsx", customPatterns)).toBe(
      true,
    );
    expect(shouldIgnoreFile("src/utils.ts", customPatterns)).toBe(false);
    expect(shouldIgnoreFile("src/testing-library.ts", customPatterns)).toBe(
      true,
    ); // contains 'test'
  });
  ...
  it("should handle complex path structures", () => {
    ...
    expect(shouldIgnoreFile("packages/app/.next/static/chunks/main.js")).toBe(
      true,
    );
